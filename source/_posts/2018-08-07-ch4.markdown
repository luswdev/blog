...
- {% label danger@問題 %}：此時 list 上將會遺失原本 process A `insert` 的節點。
...
- 最後 process B 也將 list 設為 l，此時 list 上的第一顆為 B 的資料，{% label danger@且 A 的資料遺失了 %}
...
```
- 但是我們發現，可能會有多個 CPU 執行至第五行，發現 `lk->locked` 為 `0`，接著都拿到了鎖，{% label warning@即違反了互斥 %}
